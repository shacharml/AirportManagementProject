...
int		saveManagerToFile(const AirportManager* pManager, const char* fileName)
{
	...
	while (pNode)
	{
		CHECK_0_MSG_COLSE_FILE(saveAirportToFile(pNode->key, fp), fp, Error write airport);
		pNode = pNode->next;
	}
	fclose(fp);
	return 1;
...
int		loadManagerFromFile(AirportManager* pManager, const char* fileName)
{
	...
	for (int i = 0; i < pManager->count; i++)
	{
		...
		CHECK_0_MSG_COLSE_FILE(loadAirportFromFile(pPort, fp), fp, Error loading airport from file);
